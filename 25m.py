...
# realiza una consulta a la base de datos para obtener las valoraciones de usuarios para una película específica
def calculate_user_similarity(user_ratings, other_user_ratings, all_movie_ids):
    ...
    return similarity
#toma las valoraciones de dos usuarios y calcula la similitud del coseno entre ellas, proporcionando una medida de qué tan similares son en términos de sus preferencias de películas.

def recommend_movies(connection, user_id):
    # Obtener todas las películas y sus IDs
    #obtiene todos los identificadores únicos de películas en la base de datos y almacena esos identificadores en la lista 
